...
Imports System.Data
Partial Class sw_comprasadministrador
    ...
    Sub insertacompraanterior()
        ...
        Dim id As Integer = Session("asociadocompra")
        ...
        For cont = 0 To idpaq.Count - 1
            ...
            If activo Then
                If funciones.esmisemana(Session("asociadocompra")) Then
                    'sí es mi semana
            ...
            Else
                ...
                If funciones.esmisemana(Session("asociadocompra")) Then
                    'sí es mi semana
                    Dim misiguienteactivacion As Date = funciones.misiguienteactivacion(Session("asociadocompra"))
                    If fechadecompra >= misiguienteactivacion Then
            ...
            strTeamQuery = "INSERT INTO compras(asociado, paquete, cantidad, puntos, costo, fecha, activacion, excedente, autor) VALUES(" & id.ToString & ", " & idpaq(cont).ToString & ", " & cantidades(cont).ToString & ", " & puntos(cont).ToString & ", " & costo(cont).ToString & ", '" & fechadecompra.Year & "/" & fechadecompra.Month & "/" & fechadecompra.Day & "', " & compraactivadora.ToString & ", " & excedente.ToString & ", '" & Session("idasociado").ToString & "')"
    ...
    Sub extiendeactivaciondeasociado(ByVal puntos As Integer)
        fechadecompra = CDate(Me.fecha.Text)
        Dim strConnection As String = ConfigurationManager.ConnectionStrings("conexionKisem").ConnectionString
        Dim sqlConn As MySqlConnection = New MySqlConnection(strConnection)
        Dim strTeamQuery As String = ""
        Dim cmdFetchTeam As MySqlCommand = New MySqlCommand(strTeamQuery, sqlConn)
        Dim finactivacion As Date = funciones.misiguienteactivacion(Session("asociadocompra"))
        finactivacion = DateAdd(DateInterval.Day, -1, finactivacion)
        finactivacion = DateAdd(DateInterval.Month, 1, finactivacion)
        sqlConn.Open()
        strTeamQuery = "UPDATE asociados SET  inicioactivacion='" & fechadecompra.Year.ToString & "/" & fechadecompra.Month.ToString & "/" & fechadecompra.Day.ToString & "', finactivacion='" & finactivacion.Year.ToString & "/" & finactivacion.Month.ToString & "/" & finactivacion.Day.ToString & "', PtsMes=" & puntos.ToString & " WHERE id=" & Session("asociadocompra").ToString
    ...
    Sub activaasociado(ByVal puntos As Integer)
        ...
        finactivacion = DateAdd(DateInterval.Day, -1, finactivacion)
        sqlConn.Open()
        strTeamQuery = "UPDATE asociados SET status=1, inicioactivacion='" & fechadecompra.Year.ToString & "/" & fechadecompra.Month.ToString & "/" & fechadecompra.Day.ToString & "', finactivacion='" & finactivacion.Year.ToString & "/" & finactivacion.Month.ToString & "/" & finactivacion.Day.ToString & "', PtsMes=" & puntos.ToString & " WHERE id=" & Session("asociadocompra").ToString
    ...
    Function checaactivo() As Boolean
        Dim strConnection As String = ConfigurationManager.ConnectionStrings("conexionKisem").ConnectionString
        Dim sqlConn As MySqlConnection = New MySqlConnection(strConnection)
        Dim strTeamQuery As String = "SELECT status FROM asociados WHERE id=" & Session("asociadocompra").ToString
        Dim cmdFetchTeam As MySqlCommand = New MySqlCommand(strTeamQuery, sqlConn)
    ...
    Protected Sub Page_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
        ...
        If Not IsPostBack Then
            Me.fecha.Text = Date.Today.ToString("dd/MM/yyyy")
    ...
    Sub llenagracias()
        ...
        Me.oc.Text = recuperareferencia(Session("asociadocompra")).ToString
    ...
    Sub llenatablapaquetes()
        ...
        Dim strConnection As String = ConfigurationManager.ConnectionStrings("conexionKisem").ConnectionString
        Dim sqlConn As MySqlConnection = New MySqlConnection(strConnection)
        Dim strTeamQuery As String = "SELECT id, costo, nombre FROM paquetes WHERE id>=0 and id<4"
    ...
    Private Sub manejabotones(ByVal sender As Object, ByVal e As EventArgs)
        Me.faltaasociado.Visible = False
        Dim asociado() As String = Split(Me.txtasociado.Text, " ")
        Try
            Session("asociadocompra") = CInt(asociado(0))
            fechadecompra = CDate(Me.fecha.Text)
        Catch ex As Exception
            Me.faltaasociado.Visible = True
            Exit Sub
        End Try

        If Not funciones.existeasociado(Session("asociadocompra")) Then
            Me.faltaasociado.Visible = True
            Exit Sub
        End If
    ...
    Sub insertacompra(ByVal paquetes As List(Of Integer), ByVal puntos As List(Of Integer), ByVal costo As List(Of Decimal), ByVal cantidades As List(Of Integer), ByVal activadora As Integer, ByVal excedente As Integer, Optional ByVal activacionafuturo As Integer = 0)
        ...
        If activadora = 1 And activacionafuturo = 0 Then
            Dim fechatemp As Date = funciones.misiguienteactivacion(Session("asociadocompra"))
        ...
        If statuspago = "PAGADO" Then
            fechaentrega = fechadecompra.Year & "/" & fechadecompra.Month & "/" & fechadecompra.Day

        End If


        If inicioactivacion > CDate("0001/01/01") Then
            strTeamQuery = "INSERT INTO compras(asociado, puntos,  fechaorden, activacion, excedente, statuspago, statusentrega, inicioactivacion, finactivacion, tipopago, tipoentrega, iva, envio, total, fecha, autor) VALUES(" & id.ToString & ", " & puntostotales.ToString & ", '" & fechadecompra.Year & "/" & fechadecompra.Month & "/" & fechadecompra.Day & "', " & activadora.ToString & ", " & excedente.ToString & ", '" & statuspago & "', '" & statusentrega & "', '" & inicioactivacion.ToString("yyyy/MM/dd") & "', '" & finactivacion.ToString("yyyy/MM/dd") & "', '" & tipopago & "', '" & tipoentrega & "', " & iva.ToString & ", " & envio.ToString & ", " & total.ToString & ", '" & fechaentrega & "', '" & Session("idasociado").ToString & "')"
        Else
            strTeamQuery = "INSERT INTO compras(asociado, puntos,  fechaorden, activacion, excedente, statuspago, statusentrega, tipopago, tipoentrega, iva, envio, total, fecha, autor) VALUES(" & id.ToString & ", " & puntostotales.ToString & ", '" & fechadecompra.Year & "/" & fechadecompra.Month & "/" & fechadecompra.Day & "', " & activadora.ToString & ", " & excedente.ToString & ", '" & statuspago & "', '" & statusentrega & "', '" & tipopago & "', '" & tipoentrega & "', " & iva.ToString & ", " & envio.ToString & ", " & total.ToString & ", '" & fechaentrega & "', '" & Session("idasociado").ToString & "')"
        End If
        ...
        If statuspago = "PAGADO" Then
            ...
            For i = 0 To puntos.Count - 1
                ...
                Dim bodega As Integer = funciones.buscabodega(Session("asociadocompra"))
                funciones.descuentadebodegas(bodega, paquetes(i), cantidades(i))
            Next
            funciones.subepuntos(compra, puntostotales, Session("asociadocompra"))
        ...
        'Dim mail As String = correo.micorreo(Session("asociadocompra"))
        'Dim nombreasociadocompra As String = correo.minombre(Session("asociadocompra"))
        ...
        'End With
        'Next
        'body.Append("<br/><span style='font-weight:bold;'>" & Session("asociadocompra") & " " & nombreasociadocompra & " tu compra fue exitosa.</span>")
    ...
    Sub procesacompra()
        ...
        If activo Then
            If funciones.esmisemana(Session("asociadocompra")) Then
                ...
                Dim misiguienteactivacion As Date = funciones.misiguienteactivacion(Session("asociadocompra"))
                Dim mifindeactivacion As Date = funciones.mifindeactivacion(Session("asociadocompra"))
                ' If fechadecompra >= misiguienteactivacion Or misiguienteactivacion < mifindeactivacion Then
                If fechadecompra < DateAdd(DateInterval.Day, -7, mifindeactivacion) Then
        ...
        Else
            'ESTÁ INACTIVO
            'If funciones.esmisemana(Session("asociadocompra")) And Date.Today <= funciones.misiguienteactivacion(Session("asociadocompra")) Then
            If funciones.esmisemana(Session("asociadocompra")) Then
                'sí es mi semana
                Dim misiguienteactivacion As Date = funciones.misiguienteactivacion(Session("asociadocompra"))
                If Not funciones.activacionenloquerestadelasemana(Session("asociadocompra")) Then
    ...
    Sub llenadatos()
        Dim strConnection As String = ConfigurationManager.ConnectionStrings("conexionKisem").ConnectionString
        Dim sqlConn As MySqlConnection = New MySqlConnection(strConnection)
        Dim strTeamQuery As String = "SELECT CONCAT(nombre, ' ', appaterno, ' ', apmaterno) AS nombre, CONCAT(callepaq, ' ', numpaq, ' ', intpaq) AS direccion, colpaq, cppaq, municipiopaq, estadopaq FROM asociados WHERE id=" & Session("asociadocompra").ToString
    ...
    Sub activaasociadoafuturo(ByVal puntos As Integer, ByVal inicio As Date, ByVal fin As Date)
        ...
        sqlConn.Open()
        strTeamQuery = "UPDATE asociados SET status=0, inicioactivacion='" & inicio.Year.ToString & "/" & inicio.Month.ToString & "/" & inicio.Day.ToString & "', finactivacion='" & fin.Year.ToString & "/" & fin.Month.ToString & "/" & fin.Day.ToString & "', PtsMes=" & puntos.ToString & " WHERE id=" & Session("asociadocompra").ToString
